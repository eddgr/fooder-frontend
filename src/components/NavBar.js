...
function NavBar(props) {
  ...
  const chatNav = () => {
    return (
      <>
        ...
        <h1
          id="venue"
          onClick={() => document.querySelector("#chat-box").scrollTo(0, 0)}
        >
        ...
        <Link
          onClick={() => moreInfo()}
          to={`venues/${props.currentUser.selectedVenue.id}`}
        >
  ...
  const searchIcon = () => {
    if (
      ...
      window.location.pathname.includes("/venues/")
    ) {
      return (
        <i
  ...
  const normNav = () => {
    return (
      <>
        ...
        <Link to="/profile">
          <i className="far fa-user-circle text-dark"></i>
        </Link>
        {/*
  ...
  return (
    <nav className="navbar fixed-top bg-light" style={{ height: "44px" }}>
